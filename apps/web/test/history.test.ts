...
describe("recent displayed lines", () => {
  it("keeps the last three distinct completed matches without mutating earlier history", () => {
    ...
    for (let i = 0; i < 4; i++) history = appendHistory(history, { sessionId: String(i), lineIds: [String(i), String(i)] });
  ...
  it.each([null, "bad json", "null", '{"version":2,"matches":[]}',
  ...
  it("persists once under duplicate finalization and reads newer stored history", () => {
    ...
    let writes = 0;
    const store = createHistoryStore(() => ({ getItem: () => raw, setItem: (_key, value) => { raw = value; writes++; } }));
